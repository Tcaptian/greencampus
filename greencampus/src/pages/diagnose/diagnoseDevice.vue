  ...
  <div>
    ...
    <div class="roleMessage">
      ...
      <ul v-for="(item, index) of data1" :key="item.id">
        <li>{{index+1}}</li>
        <li>{{item.diaDeviceName}}</li>
        <li>{{item.diaDeviceContent}}</li>
        <li>{{item.diaDeviceType}}</li>
        <li>{{item.time}}</li>
...
export default {
  ...
  methods: {
    ...
    handleChangePage (index) {
      ...
      axios.get('api/GreenCampusDia/allDiaDevice', {
        params: {
          pageNo: index,
          pageSize: 5
        }
