...
export default {
                  ...
                  return h('div', [
                      h('Button', {
                          ...
                          on: {
                              click: () => {
                                  this.modal1 = true
                                  this.controlData(params.row.batteryVOList)
                                  this.drawLine()
                              }
                          }
                      }, '查看详情')
  ...
  methods: {
    getAllWorkHoildayConsume () {
      axios.get('api/GreenCampus/elec/allWorkHoliday').then(res => {
          this.data1 = res.data.detail
          this.data1.forEach(item => {
            item.battery = item.batteryVOList[0].battery
          })
    ...
    controlData (showData) {
      showData.forEach(item => {
        this.agencySeriersData.push(item.battery)
